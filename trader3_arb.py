...
class Trader:
    ...
    def calc_next_price_orchids(self):
        return self.o_cache[-1], self.o_duck_cache[-1]
    ...
    def compute_orders_orchids(self, product, order_depth, acc_bid, acc_ask, conversionObservations):
        ...
        duck_bid_price = conversionObservations.bidPrice
        duck_ask_price = conversionObservations.askPrice

        sell_vol, best_sell_pr = self.values_extract(osell)
        buy_vol, best_buy_pr = self.values_extract(obuy, 1)

        cpos = self.position[product]
        # first get long pos, then convertion(neg number), ie selling to ducks
        arb_price = duck_bid_price+transportFee+exportTar
        # first get short pos, then convertion(pos number), ie buying from ducks
        rarb_price = duck_ask_price+transportFee+importTar
        arb_profit = 0
        rarb_profit = 0
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        if len(self.o_cache) == self.o_dim:
            self.o_cache.pop(0)
            self.o_duck_cache.pop(0)
        ...
        _, bs_o = self.values_extract(collections.OrderedDict(sorted(state.order_depths['ORCHIDS'].sell_orders.items())))
        _, bb_o = self.values_extract(collections.OrderedDict(sorted(state.order_depths['ORCHIDS'].buy_orders.items(), reverse=True)), 1)
        self.o_cache.append((bs_o + bb_o) / 2)

        _, bs_o_duck = state.observations.conversionObservations["ORCHIDS"].askPrice
        _, bb_o_duck = state.observations.conversionObservations["ORCHIDS"].bidPrice
        self.o_cache.append((bs_o_duck + bb_o_duck) / 2)
        ...
        if len(self.o_cache) == self.o_dim:
            next_price, next_duck_price = self.calc_next_price_orchids()
            o_lb = next_price - 1
            o_ub = next_price + 1
            o_duck_lb = next_duck_price - 1
            o_duck_ub = next_duck_price + 1

        acc_bid = {'AMETHYSTS' : 10_000, 'STARFRUIT' : sf_lb, 'ORCHIDS' : o_lb} # we want to buy at slightly below
        acc_ask = {'AMETHYSTS' : 10_000, 'STARFRUIT' : sf_ub, 'ORCHIDS' : o_ub} 
        ...
        for product in state.order_depths:
            ...
            if (product == 'AMETHYSTS'):
                ...
                orders, pp = self.compute_orders_orchids(product, order_depth, acc_bid[product], acc_ask[product], state.observations.conversionObservations[product])
